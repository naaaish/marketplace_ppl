...
<head>
    ...
    <style>
        ...
        /* Lebar Kolom Khusus */
        th:first-child, td:first-child { width: 30px; text-align: center; } /* No */
...
<table>
        <thead>
            <tr>
                ...
                <th>Rating</th>
                <th>Nama Toko</th>
                <th>Propinsi (Pemberi Rating)</th> </tr>
        </thead>
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $review->product->category ?? '-' }}</td>
                
                <td>Rp {{ number_format($review->product->price ?? 0, 0, ',', '.') }}</td>
                
                <td style="text-align: center;">{{ $review->rating }}</td>
                
                <td>{{ $review->product->seller->store_name ?? '-' }}</td>
                
                <td>{{ $review->user->province ?? 'Tidak Diketahui' }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
    ...
    <div class="footer-note">
        ***) propinsi diisikan propinsi pemberi rating (toko)
    </div>
</body>
</html>