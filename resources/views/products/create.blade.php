    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <div class="w-64 bg-[#102C54] text-white flex-shrink-0 flex flex-col">
            ...
            <nav class="flex-1 px-4 space-y-2 mt-4 sidebar-scroll overflow-y-auto">
                ...
                <a href="{{ route('products.create') }}" class="flex items-center gap-3 px-4 py-3 bg-white text-[#102C54] rounded-lg font-semibold shadow-md transition">
                ...
                <form action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data" class="max-w-4xl mx-auto space-y-8">
                        ...
                        <div class="space-y-5">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                ...
                                <div>
                                    ...
                                    <input type="number" name="weight" value="{{ old('weight') }}" class="form-input" required>
                                </div>
                            </div>
                            <div>
                    ...
                    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                        <h3 class="text-lg font-bold text-gray-800 mb-4 border-b pb-2 flex justify-between items-center">
                            <span>Variasi Produk</span> <span class="text-xs font-normal text-gray-500">(Opsional)</span>
                        </h3>
    ...
    <script>
        ...
        function toggleStatus() {
            ...
            if (toggle.classList.contains('active')) {
                text.textContent = 'Aktif'; input.value = 'active'; toggle.style.background = '#22c55e';
            } else {
                text.textContent = 'Nonaktif'; input.value = 'inactive'; toggle.style.background = '#cbd5e1';
            }
        }
        let variantCount = 0;
        function addVariant() {
            ...
            const html = `
                <div class="grid grid-cols-1 md:grid-cols-12 gap-3 items-end bg-gray-50 p-4 rounded-lg border border-gray-200" id="variant-${variantCount}">
                    <div class="md:col-span-4"><label class="text-xs text-gray-500 mb-1 block">Nama Variasi</label><input type="text" name="variants[${variantCount}][name]" class="form-input text-sm"></div>
                    <div class="md:col-span-3"><label class="text-xs text-gray-500 mb-1 block">Harga</label><input type="number" name="variants[${variantCount}][price]" class="form-input text-sm"></div>
                    <div class="md:col-span-2"><label class="text-xs text-gray-500 mb-1 block">Stok</label><input type="number" name="variants[${variantCount}][stock]" class="form-input text-sm"></div>
                    <div class="md:col-span-2"><label class="text-xs text-gray-500 mb-1 block">SKU</label><input type="text" name="variants[${variantCount}][sku]" class="form-input text-sm"></div>
                    <div class="md:col-span-1 text-right"><button type="button" onclick="document.getElementById('variant-${variantCount}').remove()" class="text-red-500 hover:text-red-700 bg-white p-2 rounded border border-red-200 hover:bg-red-50"><i class="fas fa-trash"></i></button></div>
                </div>`;
            container.insertAdjacentHTML('beforeend', html);
            variantCount++;
        }
        @if(session('success'))
            Swal.fire({ icon: 'success', title: 'Berhasil!', text: "{{ session('success') }}", confirmButtonColor: '#102C54', confirmButtonText: 'Oke' })
            .then((result) => { if (result.isConfirmed) { window.location.href = "{{ route('seller.dashboard') }}"; } });
        @endif
    </script>
</body>
</html>